...
int crc_x (int data, int data_length, int crc_length){
  ...
  for (int i = telem_length-poly_length; i >= 0; i--){
    if (bitRead(tmp_telem, (i+poly_length) == 1)) // if the MSB of the selected telem subset is a 1
    {
      // then operate on that subset
      tmp_quot = (tmp_telem >> i) ^ poly; // for the current position divide th the poly
      ...
      tmp_telem = tmp_quot << i & tmp_telem; // bitwise AND to prep the next iteration of the crc calculation
    } // end if
  } // end for
